            ...
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">
                ...
                <el-tag
                  size="medium"
                  type="info"
                  v-if="!scope.row.enableRemoveNoneReader && !scope.row.enableDisableNoneReader"
                >不做处理</el-tag>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="300" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                icon="el-icon-check"
                type="text"
                :loading="scope.row.startBtnLoading"
                v-if="!scope.row.enable"
              ...
              <el-button
                ...
                type="text"
                style="color: #909399"
                @click="editStreamProxy(scope.row)"
              >编辑</el-button>
              <el-button
                size="medium"
                icon="el-icon-delete"
...
export default {
  ...
  methods: {
      ...
      this.$axios({
        ...
        .then(function(res) {
          if (res.data.code === 0) {
            ...
            that.streamProxyListLoading = false;
          }
        })
    ...
    addStreamProxy: function() {
      const list = this.$refs.deviceGroup.getGroup();
      // if(list.length == 0) {
      //   this.$message.warning("暂无分组数据");
      // }
      this.$refs.streamProxyEdit.openDialog(
        null,
        this.refeshRequest,
        "add",
        list
            ...
            if (res.data.data.length > 0) {
              this.$refs.onvifEdit.openDialog(res.data.data, url => {
                if (url != null) {
                  ...
                  this.$refs.streamProxyEdit.openDialog(
                    { type: "default", url: url, srcUrl: url },
                    this.initData()
                  );
                }
              });
    ...
    refeshRequest() {
      ...
      this.changeGroupTimer = setTimeout(() => {
        this.streamProxyListLoading = true;
        this.startUpdateList();
      }, 500);
    }
  }
