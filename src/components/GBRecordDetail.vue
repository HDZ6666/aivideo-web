          ...
          <div class="record-list-box" v-loading="recordsLoading" :style="recordListStyle">
            <ul v-if="detailFiles.length >0" class="infinite-list record-list">
              <li
                ...
                :key="item.filePath"
                class="infinite-list-item record-list-item"
              >
      ...
      <el-main style="padding-bottom: 10px;">
        ...
        <div class="player-option-box">
          ...
          <div class="slider-val-box">
            <div
              class="slider-val"
              v-for="(item,index) of detailFiles"
              :key="index"
...
export default {
  ...
  methods: {
    dateChange() {
      ...
      this.setTime(
        this.chooseDate + " 00:00:00",
        this.chooseDate + " 23:59:59"
      );
    ...
    setTime: function(startTime, endTime) {
      ...
      let start =
        (new Date(this.startTime).getTime() -
          new Date(this.chooseDate + " 00:00:00").getTime()) /
        1000;
      let end =
      ...
      if (this.streamId !== "") {
        ...
        this.$axios({
          ...
          if (res.data.code === 0) {
            ...
            this.hasAudio =
              this.streamInfo.tracks && this.streamInfo.tracks.length > 1;
          } else {
            this.$message({
              showClose: true,
              message: res.data.msg,
    ...
    downloadRecord: function(row) {
      if (!row) {
        let startTimeStr = moment(
          new Date(this.chooseDate + " 00:00:00").getTime() +
            this.playTime[0] * 1000
        ).format("YYYY-MM-DD HH:mm:ss");
        let endTimeStr = moment(
          new Date(this.chooseDate + " 00:00:00").getTime() +
            this.playTime[1] * 1000
        ).format("YYYY-MM-DD HH:mm:ss");
      ...
      if (this.streamId !== "") {
        ...
        this.$axios({
          ...
          if (res.data.code === 0) {
            ...
            this.$refs.recordDownload.openDialog(
              this.deviceId,
              this.channelId,
              streamInfo.app,
              streamInfo.stream,
    ...
    getDataLeft(item) {
      let timeForFile = this.getTimeForFile(item);
      let differenceTime =
        timeForFile[0].getTime() -
        new Date(this.chooseDate + " 00:00:00").getTime();
      return (
        parseFloat(
          (differenceTime - this.sliderMIn * 1000) /
            ((this.sliderMax - this.sliderMIn) * 1000)
        ) * 100
      );
    },
    getUrlByStreamInfo() {
    ...
    playTimeChange(val) {
      ...
      let startTimeStr = moment(
        new Date(this.chooseDate + " 00:00:00").getTime() + val[0] * 1000
      ).format("YYYY-MM-DD HH:mm:ss");
      let endTimeStr = moment(
        new Date(this.chooseDate + " 00:00:00").getTime() + val[1] * 1000
      ).format("YYYY-MM-DD HH:mm:ss");
    ...
    setSliderFit() {
      if (this.sliderMIn === 0 && this.sliderMax === 86400) {
        if (this.detailFiles.length > 0) {
          ...
          let lastTimeForFile = this.getTimeForFile(
            this.detailFiles[this.detailFiles.length - 1]
          );
          let timeNum =
            timeForFile[0].getTime() -
            new Date(this.chooseDate + " " + "00:00:00").getTime();
          let lastTimeNum =
            lastTimeForFile[1].getTime() -
            new Date(this.chooseDate + " " + "00:00:00").getTime();

          this.playTime = parseInt(timeNum / 1000);
          this.sliderMIn = parseInt(
            timeNum / 1000 - ((timeNum / 1000) % (60 * 60))
          );
          this.sliderMax =
            parseInt(lastTimeNum / 1000 - ((lastTimeNum / 1000) % (60 * 60))) +
            60 * 60;
