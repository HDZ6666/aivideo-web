    ...
    <el-container v-loading="loading" style="height: 91vh;" element-loading-text="拼命加载中">
      ...
      <el-container>
        <el-header height="5vh" style="text-align: left;font-size: 17px;line-height:5vh">
          分屏:
          <i class="el-icon-full-screen btn" :class="{ active: spilt == 1 }" @click="spiltScreen(1)" />
          <i class="el-icon-menu btn" :class="{ active: spilt == 4 }" @click="spiltScreen(4)" />
          <i class="el-icon-s-grid btn" :class="{ active: spilt == 9 }" @click="spiltScreen(9)" />
        </el-header>
        <el-main style="padding: 0;">
          <div
            style="width: 99%;height: 85vh;display: flex;flex-wrap: wrap;background-color: #000;"
          >
            <div
              v-for="i in spilt"
              :key="i"
...
import DeviceTree from "./common/DeviceTree2.vue";
