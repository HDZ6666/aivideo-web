<template>
  <el-dialog v-model="open" title="任务编辑" width="min(960px, 96vw)" top="4vh" append-to-body destroy-on-close @closed="reset">
    <div class="patrol-dialog-body">
      ...
      <el-form-item label="所选时间段" prop="selectedHours">
...
function openDialog(row) {
  ...
  form.taskRule = row.taskRule || '0'
  form.selectedHours = row.selectedHours || []
  selectedDeviceIds.value = []
  form.selectedCameras = ''
  activeTab.value = row.playerAction === 'proxy' ? 'proxy' : 'national'
  open.value = true
}
...
function submit() {
  formRef.value?.validate((valid) => {
    ...
    request({
      method: 'post',
      url: '/api/patrol/route_task/edit',
      data: { ...form }
    })
      .then(() => {
