<template>
  <div id="devicePosition" style="width: 100vw; height: 91vh;">
    <el-container v-if="onOff" style="height: 91vh;" v-loading>
...
export default {
  ...
  created() {
    if (this.$route.query.deviceId) {
      ...
      setTimeout(() => {
        // 延迟以等待地图加载完成 TODO 后续修改为通过是实际这；状态加回调完成
        this.deviceService.getAllChannel(
          false,
          false,
          this.$route.query.deviceId,
          this.channelsHandler
        );
      }, 1000);
    }
  ...
  methods: {
    clickEvent: function(device, data, isCatalog) {
      ...
      if (data.channelId && !isCatalog) {
        ...
        if (data[this.longitudeStr] * data[this.latitudeStr] === 0) {
          ...
          this.$refs.map.panTo(
            [data[this.longitudeStr], data[this.latitudeStr]],
            mapParam.maxZoom
          );
        }
      }
        ...
        items: [
          {
            ...
            onClick: () => {
              ...
              if (data.channelId && data.subCount > 0) {
                // 点击子目录
                this.deviceService.getAllSubChannel(
                  false,
                  data.deviceId,
                  data.channelId,
                  this.channelsHandler
    ...
    channelsHandler: function(channels) {
      if (channels.length > 0) {
        ...
        if (params.length === 1) {
          this.$refs.map.panTo(
            [channels[0][this.longitudeStr], channels[0][this.latitudeStr]],
            mapParam.maxZoom
          );
    ...
    featureClickEvent: function(channels) {
      ...
      this.$nextTick(() => {
        let position = [
          this.channel[this.longitudeStr],
          this.channel[this.latitudeStr]
        ];
        this.infoBoxId = this.$refs.map.openInfoBox(
          position,
          this.$refs.infobox,
          [0, -50]
        );
      ...
      this.$refs.queryTrace.openDialog(data, channelPositions => {
        ...
        if (channelPositions.length === 0) {
          ...
          for (let i = 0; i < channelPositions.length; i++) {
            if (
              channelPositions[i][this.longitudeStr] *
                channelPositions[i][this.latitudeStr] >
              0
            ) {
              positions.push([
                channelPositions[i][this.longitudeStr],
