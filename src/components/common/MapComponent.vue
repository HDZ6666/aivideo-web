...
export default {
  ...
  methods: {
    init() {
      let center = fromLonLat([116.4102, 39.915119]);
      if (mapParam.center) {
    ...
    addMarker(position) {
      ...
      olMap.addLayer(vectorLayer);
      this.markerId = vectorLayer
        .getSource()
        .getFeatureById(markerFeature.getId())
        .getId();
    },
    handleMapClick(event) {
      olMap.on("click", this.handleMapClick);
      const coordinate = olMap.getCoordinateFromPixel(event.pixel);
      this.addMarker(coordinate);
    }
  },
  destroyed() {
    olMap.un("click", this.handleMapClick);
    // if (this.jessibuca) {
