...
const addPolyline = (path, options = {}) => {
  ...
  const polyline = new window.AMap.Polyline({
    ...
    strokeDasharray: options.strokeDasharray || [10, 5],
    ...options
  })
...
// 自适应显示所有覆盖物
const setFitView = (overlays = null) => {
  if (!map.value) return

  const allOverlays = overlays || [...markers.value, ...polylines.value]
