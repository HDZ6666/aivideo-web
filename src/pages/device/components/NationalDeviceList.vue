...
import type { INationalDevice } from '@/api/device'
import { LoadingState } from '@/components/loading-state'
import { useDeviceStore } from '@/store/device'

interface Props {
  deviceList: INationalDevice[]
  loading?: boolean
}
...
function getDeviceStatusText(device: INationalDevice) {
  return device.onLine ? '在线' : '离线'
}

function getDeviceTypeBadgeClasses(device: INationalDevice) {
  return device.onLine ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'
}

function handleDeviceSelect(device: INationalDevice) {
  deviceStore.setSelectedNationalDevice(device)
  uni.navigateTo({
    url: '/pages/device/national-detail',
  })
}
</script>

<template>
  <view class="national-device-list">
    <view class="device-list-section">
      <!-- 加载状态 -->
      <LoadingState
        v-if="props.loading && props.deviceList.length === 0"
        state="loading"
        loading-text="正在加载国标设备..."
        compact
      />
      ...
      <template v-else>
        <view
          ...
          :key="device.deviceId"
          class="device-list-item mb-24rpx"
          :class="{ 'device-list-item--offline': !device.onLine }"
          @click="handleDeviceSelect(device)"
        >
          <view class="device-card rounded-24rpx bg-white p-32rpx shadow-gray-200/60 shadow-lg transition-all duration-300">
            <view
              class="card-gradient absolute h-128rpx w-128rpx rounded-full opacity-10 -right-32rpx -top-32rpx"
              :class="device.onLine ? 'bg-gradient-to-br from-green-400 to-green-600' : 'bg-gradient-to-br from-red-400 to-red-600'"
            />
              ...
              <view class="device-header mb-24rpx flex items-start justify-between">
                ...
                <view class="status-indicator flex items-center">
                  ...
                  <text
                    ...
                    :class="device.onLine ? 'text-green-600' : 'text-red-600'"
                  >
