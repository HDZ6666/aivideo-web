...
import { init, ZRender, Circle, Line, Group, Element, Polygon } from "zrender";
...
class PolyBrush2D extends React.Component<PolyBrush2DProps, any> {
	static defaultProps = {
		width: 400,
		height: 300,
		color: "#fff",
	...
	private zr: ZRender | any;
	private color: string | any = this.props.color;
	private type: string | any = this.props.type;
	// 当前正在绘制的线段是否和已绘制的线段交叉，仅限多边形
	private isIntersect: boolean = false;
	// 是否开始绘制
	private isStart: boolean = false;
	// 一个完整的图形为一个group
	private group: Group | any;
	// 当前绘制图形的元数据
	...
	UNSAFE_componentWillReceiveProps(nextProps: PolyBrush2DProps) {
	...
	drawPolygon = (event: MouseEvent) => {
		...
		const junction = this.junction(offsetX, offsetY, 2, this.color);
	...
	completePolygon = (event: MouseEvent) => {
		...
		if (!isIntersect) {
			...
			if (!this.group) return;
			const points = this.group.children().reduce((array: any, item: Group) => {
				if (item.type === "circle") {
					array.push([item.shape.cx, item.shape.cy]);
				}
				return array;
			}, []);
			// 根据绘制的点，绘制区域
			const polygon = this.polygon(points);
			this.zr.add(polygon); // 添加图形
			this.remove(this.group); // 清除之前的点和线组合
			this.dataSource.push(polygon); //添加到返回集中
	...
	// 连接线
	line = (x1: number, y1: number, x2: number, y2: number, stroke: string): Line => {
		return new Line({
			shape: { x1, y1, x2, y2 },
