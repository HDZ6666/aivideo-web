<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:comprehensive:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:comprehensive:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="comprehensiveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用户注册时长" align="center" prop="userRegistrationDuration" />
      <el-table-column label="用户模板总数" align="center" prop="userTemplateNum" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="用户注册时长" prop="userRegistrationDuration">
          <el-input v-model="form.userRegistrationDuration" placeholder="请输入用户注册时长" />
        </el-form-item>
        <el-form-item label="用户模板总数" prop="userTemplateNum">
          <el-input v-model="form.userTemplateNum" placeholder="请输入用户模板总数" />
        </el-form-item>
      </el-form>
    ...
    return {
      ...
      rules: {
        ...
        userRegistrationDuration: [
          { required: true, message: "用户注册时长不能为空", trigger: "blur" }
        ],
        userTemplateNum: [
          { required: true, message: "用户模板总数不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    reset() {
      this.form = {
        ...
        userName: undefined,
        userRegistrationDuration: undefined,
        userTemplateNum: undefined,
        createBy: undefined,
    ...
    handleUpdate(row) {
      ...
      getComprehensive(userId).then(response => {
        this.loading = false;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户综合查询编号为"' + userIds + '"的数据项？').then(() => {
        this.loading = true;
        return delComprehensive(userIds);
      }).then(() => {
        this.loading = false;
        this.getList();
