...
import { RulesForm, type RulesQuery } from '@/api/model/alarm/notificationsModel';

const apiClient = getApiClient();

const url = '/api/alarm/v2/notifications/';
...
export function delRules(ids: number[]) {
  return apiClient.POST('/api/alarm/v2/notifications/delete', { body: { ids: ids } });
}
...
export function getRules(id: number) {
  return apiClient.GET('/api/alarm/v2/notifications/{id}', { params: { path: { id: id } } });
}
